...
public class EventController {
    ...
    public ModelAndView editEvent(@PathVariable("id") int id, HttpSession session) {
    ...
    public ModelAndView deleteEvent(@PathVariable("id") int id , HttpSession session) {
        this.eventService.deleteEventById(id);
        return new ModelAndView("redirect:/events/admin"); 
    }


    @GetMapping("/user/event_details/{id}")
    public ModelAndView viewEventDetails(@PathVariable("id") int id, HttpSession session) {
        Event event = eventService.findEventById(id);
        ModelAndView mav = new ModelAndView("/user/eventDetails.html");
        mav.addObject("event", event);
        return mav;
    }    
}